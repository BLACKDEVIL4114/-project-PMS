...
class ProjectMonitorApp:
    ...
    def add_member_modal(self):
        ...
        try:
            ...
            cur.execute("SELECT name FROM employee WHERE lower(COALESCE(role, '')) = 'team leader' ORDER BY name")
            team_leader_options = [r[0] for r in cur.fetchall()]
            con.close()
        except:
            avail = []
            team_leader_options = []
        ...
        selected_team_leader = CURRENT_USER_NAME if CURRENT_USER_ROLE.lower() == 'team leader' else ""
        if CURRENT_USER_ROLE.lower() != 'team leader':
            tl_wrap = Frame(pick_wrap, bg=INPUT_BG)
            tl_wrap.pack(side=LEFT, padx=(0, 15))
            c_fast_tl = ttk.Combobox(tl_wrap, values=team_leader_options, font=('Segoe UI', 11), style='Employee.TCombobox', state="readonly", width=22)
            c_fast_tl.pack(fill=X, padx=2, pady=2)
            if team_leader_options:
                c_fast_tl.set(team_leader_options[0])
        else:
            c_fast_tl = None
        ...
        def fast_assign():
            ...
            if target and target != "--- Select Employee ---" and target != "No unassigned employees found":
                try:
                    target_team_leader = CURRENT_USER_NAME if CURRENT_USER_ROLE.lower() == 'team leader' else (c_fast_tl.get().strip() if c_fast_tl else "")
                    if not target_team_leader:
                        messagebox.showwarning("Assign Team Leader", "Please select a team leader before onboarding the employee.")
                        return
                    c = sqlite3.connect(get_db_path())
                    cu = c.cursor()
                    cu.execute("UPDATE employee SET reporting_manager=? WHERE name=?", (target_team_leader, target))
                    c.commit(); self.refresh_current_panel(); c.close()
                    self.refresh_members()
                    t.destroy()
                    messagebox.showinfo("Success", f"{target} has been assigned under {target_team_leader}.")
        ...
        if show_manual:
            ...
            tl_holder = make_field(form, "Reporting Team Leader", 2, 0)
            reporting_tl_var = StringVar()
            c_reporting_tl = ttk.Combobox(tl_holder, textvariable=reporting_tl_var, values=team_leader_options, state="readonly", style='Employee.TCombobox')
            c_reporting_tl.pack(fill=X, padx=2, pady=2)
            if team_leader_options:
                c_reporting_tl.set(team_leader_options[0])
        else:
            ...
            def remove_member():
                ...
                if messagebox.askyesno("Confirm Removal", f"Are you sure you want to remove {name} from your team?", parent=t):
                    messagebox.showwarning("Reassignment Required", "Employees must stay mapped to a team leader. Please use Assign Team Leader to move this member to another leader.", parent=t)
        ...
        def save():
            try:
                ...
                reporting_manager = CURRENT_USER_NAME if CURRENT_USER_ROLE.lower() == 'team leader' else (reporting_tl_var.get().strip() if 'reporting_tl_var' in locals() else "")
                if role == "Team Member" and not reporting_manager:
                    messagebox.showerror("Error", "Please select a team leader for this employee.", parent=t)
                    con.close()
                    return
                
                # Default password is mobile
                pw = hashlib.sha256(mobile.encode()).hexdigest()
                cur.execute("INSERT INTO employee (name, mobile, email, department, password, role, reporting_manager) VALUES (?,?,?,?,?,?,?)",
                            (full_name, mobile, email, department, pw, role, reporting_manager))
                con.commit(); self.refresh_current_panel()
                con.close()
                self.refresh_members()
                t.destroy()
    ...
    def update_member_modal(self):
        ...
        con_tl = sqlite3.connect(get_db_path())
        cur_tl = con_tl.cursor()
        cur_tl.execute("SELECT name FROM employee WHERE lower(COALESCE(role, ''))='team leader' ORDER BY name")
        tl_names = [r[0] for r in cur_tl.fetchall()]
        cur_tl.execute("SELECT reporting_manager FROM employee WHERE id=?", (mem_id,))
        current_reporting_manager_row = cur_tl.fetchone()
        con_tl.close()
        current_reporting_manager = current_reporting_manager_row[0] if current_reporting_manager_row else ""

        Label(f, text="Reporting Team Leader", bg=CONTENT_BG, fg=TEXT_WHITE).pack(anchor=W, pady=(5,0))
        reporting_manager_var = StringVar(value=current_reporting_manager if current_reporting_manager else (tl_names[0] if tl_names else ""))
        c_reporting_tl = ttk.Combobox(f, textvariable=reporting_manager_var, values=tl_names, state="readonly")
        c_reporting_tl.pack(fill=X, pady=(0, 10))
        ...
        def save():
            try:
                ...
                new_reporting_manager = reporting_manager_var.get().strip()
                if new_role not in ("Team Leader", "Project Manager") and not new_reporting_manager:
                    con.close()
                    messagebox.showerror("Error", "Please assign a team leader for this employee.")
                    return
                if new_role in ("Team Leader", "Project Manager"):
                    new_reporting_manager = ""
                cur.execute("UPDATE employee SET name=?, mobile=?, email=?, department=?, role=?, reporting_manager=? WHERE id=?",
                            (entries["Name"].get(), entries["Mobile"].get(), entries["Email"].get(), 
                             entries["Department"].get(), new_role, new_reporting_manager, mem_id))
                con.commit(); self.refresh_current_panel()
                con.close()
                self.refresh_members()
                t.destroy()
        ...
        if CURRENT_USER_ROLE.lower() == 'team leader':
            ...
            def unlink_from_me():
                messagebox.showwarning("Reassignment Required", "Employees must report to a team leader. Please reassign this member to another team leader instead of unlinking.")
            ...
            Button(btn_row, text="Reassign Required", command=unlink_from_me, bg=ACCENT_RED, fg=TEXT_WHITE, font=('Segoe UI', 10, 'bold'), relief=FLAT).pack(side=LEFT, expand=True, fill=X, padx=5)
    ...
    def add_task_modal(self):
        ...
        if role == 'team leader':
            ...
            cur.execute("""
                SELECT name
                FROM employee
                WHERE lower(COALESCE(reporting_manager, '')) = lower(?)
                  AND lower(COALESCE(role, '')) NOT IN ('admin', 'project manager', 'team leader')
                ORDER BY name
            """, (CURRENT_USER_NAME,))
            members = [r[0] for r in cur.fetchall()]
        else:
    ...
    def load_review_tasks(self):
        ...
        if hasattr(self, '_auto_refresh_timer') and self._auto_refresh_timer:
            try:
                ...
                pass
            self._auto_refresh_timer = None
        self._auto_refresh_timer = self.root.after(
            15000,
            lambda: self.current_page == 'review_tasks' and self.load_review_tasks()
        )
        ...
        try:
            con = sqlite3.connect(get_db_path())
            cur = con.cursor()
            # Show review submissions for this leader's projects and direct reports.
            cur.execute("""
                SELECT t.id, t.title, p.name, t.assigned_to, t.status 
                FROM tasks t 
                LEFT JOIN projects p ON t.project_id = p.id 
                WHERE t.status IN ('Pending Approval', 'Review', 'Submit for Review')
                  AND (
                      lower(trim(COALESCE(p.team_leader, ''))) = lower(trim(?))
                      OR lower(trim(COALESCE(t.assigned_to, ''))) IN (
                          SELECT lower(trim(name))
                          FROM employee
                          WHERE lower(trim(COALESCE(reporting_manager, ''))) = lower(trim(?))
                      )
                  )
                ORDER BY t.id DESC
    ...
    def load_emp_dashboard(self):
        ...
        def create_saas_card(parent, title, val, sub, color, icon, progress=None):
            ...
            icon_lbl = Label(top, text=icon, font=('Segoe UI', 14, 'bold'), bg=CARD_BG, fg=color)
            icon_lbl.pack(side=LEFT)
            
            title_lbl = Label(top, text=title.upper(), font=('Segoe UI', 8, 'bold'), bg=CARD_BG, fg=MUTED_TEXT)
            title_lbl.pack(side=LEFT, padx=10)
            
            val_lbl = Label(card, text=str(val), font=('Segoe UI', 32, 'bold'), bg=CARD_BG, fg=TEXT_WHITE)
            val_lbl.pack(anchor=W, pady=(16, 4))
            ...
            sub_lbl = Label(card, text=sub, font=('Segoe UI', 9), bg=CARD_BG, fg=color)
            sub_lbl.pack(anchor=W)
            
            # Interactive hover
            def on_enter(e):
                card.config(highlightbackground=color, bg="#1e2540")
                top.config(bg="#1e2540")
                icon_lbl.config(bg="#1e2540")
                title_lbl.config(bg="#1e2540")
                val_lbl.config(bg="#1e2540")
                sub_lbl.config(bg="#1e2540")
                if p_wrap: p_wrap.config(bg="#0f172a")
                
            def on_leave(e):
                card.config(highlightbackground=BORDER_COLOR, bg=CARD_BG)
                top.config(bg=CARD_BG)
                icon_lbl.config(bg=CARD_BG)
                title_lbl.config(bg=CARD_BG)
                val_lbl.config(bg=CARD_BG)
                sub_lbl.config(bg=CARD_BG)
                if p_wrap: p_wrap.config(bg="#1e293b")
        ...
        try:
            ...
            t_card.grid(row=0, column=0, sticky="nsew", padx=(0, 12), pady=(0, 12))
            ...
            # Employee Profile & Quick Actions Card
            p_card = Frame(grid, bg=CARD_BG, padx=30, pady=30, highlightbackground=BORDER_COLOR, highlightthickness=1)
            p_card.grid(row=0, column=1, sticky="nsew", padx=(12, 0), pady=(0, 12))
            
            # Avatar circle
            avatar_lbl = Label(p_card, text=CURRENT_USER_NAME[:2].upper(), font=('Segoe UI', 16, 'bold'), bg=ACCENT_BLUE, fg=WHITE, width=4, height=2)
            avatar_lbl.pack(anchor=W)
            
            p_info = Frame(p_card, bg=CARD_BG)
            p_info.pack(fill=X, pady=(15, 20))
            Label(p_info, text=CURRENT_USER_NAME.upper(), font=('Segoe UI', 12, 'bold'), bg=CARD_BG, fg=TEXT_WHITE).pack(anchor=W)
            Label(p_info, text="TEAM MEMBER", font=('Segoe UI', 8, 'bold'), bg=CARD_BG, fg=ACCENT_BLUE).pack(anchor=W, pady=(2, 0))
            
            Label(p_card, text="MONTHLY PROGRESS", font=('Segoe UI', 8, 'bold'), bg=CARD_BG, fg=MUTED_TEXT).pack(anchor=W)
            tp_wrap = Frame(p_card, bg="#1e293b", height=6)
            tp_wrap.pack(fill=X, pady=(6, 15))
            tp_fill = Frame(tp_wrap, bg=ACCENT_BLUE, height=6)
            tp_fill.place(x=0, y=0, relwidth=0.75)
            
            Label(p_card, text="QUICK ACTIONS", font=('Segoe UI', 8, 'bold'), bg=CARD_BG, fg=MUTED_TEXT).pack(anchor=W, pady=(10, 8))
            
            def create_quick_btn(parent, text, cmd):
                b = Button(parent, text=text, bg="#1e2540", fg=TEXT_WHITE, font=('Segoe UI', 8, 'bold'), relief=FLAT, padx=15, pady=8, command=cmd)
                b.pack(fill=X, pady=4)
                b.bind("<Enter>", lambda e: b.config(bg=ACCENT_BLUE))
                b.bind("<Leave>", lambda e: b.config(bg="#1e2540"))
                
            create_quick_btn(p_card, "📝  LOG TIME & ATTENDANCE", self.log_time_window)
            create_quick_btn(p_card, "✈  REQUEST LEAVE", self.request_leave_window)
            create_quick_btn(p_card, "❓  RAISE NEW QUERY", self.raise_new_query_window)
                
            # Recent Activity Card
            a_card = Frame(grid, bg=CARD_BG, padx=30, pady=30, highlightbackground=BORDER_COLOR, highlightthickness=1)
            a_card.grid(row=1, column=0, sticky="nsew", padx=(0, 12), pady=(12, 0))
            ...
            # Work-Life & Attendance Card
            w_card = Frame(grid, bg=CARD_BG, padx=30, pady=30, highlightbackground=BORDER_COLOR, highlightthickness=1)
            w_card.grid(row=1, column=1, sticky="nsew", padx=(12, 0), pady=(12, 0))
            
            dt_today = datetime.now().strftime("%Y-%m-%d")
            cur.execute("SELECT clock_in, status FROM attendance WHERE name=? AND date=?", (CURRENT_USER_NAME, dt_today))
            att_row = cur.fetchone()
            
            Label(w_card, text="TODAY'S STATUS", font=('Segoe UI', 10, 'bold'), bg=CARD_BG, fg=TEXT_WHITE).pack(anchor=W)
            
            status_badge = Frame(w_card, padx=12, pady=8, highlightthickness=0)
            status_badge.pack(anchor=W, pady=12)
            
            if att_row:
                c_in, c_stat = att_row[0], att_row[1]
                status_badge.config(bg="#1c3d2e")
                Label(status_badge, text=f"✓  CLOCKED IN AT {c_in}", font=('Segoe UI', 8, 'bold'), bg="#1c3d2e", fg=ACCENT_GREEN).pack()
            else:
                status_badge.config(bg="#3a251c")
                Label(status_badge, text="⚠  NOT CLOCKED IN YET", font=('Segoe UI', 8, 'bold'), bg="#3a251c", fg=ACCENT_ORANGE).pack()
                
            Label(w_card, text="LEAVE BALANCE OVERVIEW", font=('Segoe UI', 8, 'bold'), bg=CARD_BG, fg=MUTED_TEXT).pack(anchor=W, pady=(15, 8))
            
            cur.execute("SELECT COUNT(*) FROM leave_requests WHERE member_name=? AND status='Approved'", (CURRENT_USER_NAME,))
            approved_leaves = cur.fetchone()[0] or 0
            vacation_left = max(0, 15 - approved_leaves)
            sick_left = 8
            
            def create_leave_row(parent, label, count, color):
                row = Frame(parent, bg=CARD_BG)
                row.pack(fill=X, pady=4)
                Label(row, text=label, font=('Segoe UI', 9), bg=CARD_BG, fg=TEXT_WHITE).pack(side=LEFT)
                Label(row, text=f"{count} Days Left", font=('Segoe UI', 9, 'bold'), bg=CARD_BG, fg=color).pack(side=RIGHT)
                
            create_leave_row(w_card, "Vacation Days", vacation_left, ACCENT_BLUE)
            create_leave_row(w_card, "Sick Leaves", sick_left, ACCENT_GREEN)
            create_leave_row(w_card, "Personal Leaves", 5, ACCENT_ORANGE)
    ...
    def _refresh_emp_team_data(self):
        ...
        try:
            ...
            # 1. Resolve the employee's active team-leader context from project work first.
            cur.execute("""
                SELECT DISTINCT TRIM(COALESCE(p.team_leader, ''))
                FROM tasks t
                LEFT JOIN projects p ON p.id = t.project_id
                WHERE lower(COALESCE(t.assigned_to, '')) = lower(?)
                  AND TRIM(COALESCE(p.team_leader, '')) != ''
            """, (CURRENT_USER_NAME,))
            ...
            if active_team_leaders:
                ...
                cur.execute(f"""
                    ...
                    FROM employee e
                    JOIN tasks t ON lower(COALESCE(t.assigned_to, '')) = lower(e.name)
                    LEFT JOIN projects p ON p.id = t.project_id
