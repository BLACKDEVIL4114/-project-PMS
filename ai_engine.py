...
class OnlinePerformanceAI:
    ...
    def load_data(self):
        """Fetch all historical performance records for batch training"""
        try:
            con = sqlite3.connect(DB_PATH)
            con.row_factory = sqlite3.Row
            cur = con.cursor()
            cur.execute("SELECT * FROM performance_history")
...
class PerformanceAI(OnlinePerformanceAI):
    ...
    def train(self):
        ...
        result = {
            ...
            "accuracy": f"{round(float(accuracy_percentage), 2)}%",
            "records_trained": len(df),
            "framework": "LightGBM" if self.config.get('model_type') == 'LightGBM Regressor' else "scikit-learn ensemble",
