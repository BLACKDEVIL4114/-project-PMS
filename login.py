...
def init_login_db():
    ...
    try:
        ...
        db_path = os.path.join(base, 'employee.db')
...
def run_login():
    ...
    state = {"mode": "employee", "action": None, "login_in_progress": False, "last_sidebar_size": (0, 0)}
    image_refs = {}
    _resize_job = None
    ...
    def _on_sidebar_resize(e):
        ...
        _resize_job = login_window.after(100, render_sidebar_background)
    ...
    main_area.bind("<Configure>", lambda e: (draw_grid(), main_area.update_idletasks()))
    ...
    def login():
        ...
        # Ignore placeholders
        if username == "Enter your first name": username = ""
        ...
        try:
            ...
            cursor.execute("SELECT password, name, role FROM employee WHERE name = ?", (username,))
            row = cursor.fetchone()
            
            if not row:
                # 2. Try prefix match if no exact match (User only entered first name)
                cursor.execute("SELECT password, name, role FROM employee WHERE name LIKE ? ORDER BY LENGTH(name) ASC LIMIT 1", (username + " %",))
                row = cursor.fetchone()
    ...
    def open_signup():
        ...
        def do_signup():
            ...
            try:
                hashed = hashlib.sha256(p1.encode()).hexdigest()
                doj = date.today().strftime('%Y-%m-%d')
                tm = datetime.now().strftime('%H:%M:%S')
                con = sqlite3.connect('employee.db')
                cur = con.cursor()
                cur.execute("""INSERT INTO employee (name, mobile, email, address, gender, dob, date, time, department, password, role)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                            (name, contact, email, 'N/A', 'N/A', dob, doj, tm, dept, hashed, target_role))
                con.commit()
                con.close()
        ...
        try:
            ...
            if action == "RUN_APP":
                try:
                    ...
                    messagebox.showerror("Application Fault", f"An unexpected error occurred during startup:\n{e}\n\nDetails:\n{err}")
            else:
                break
        except Exception as e:
            messagebox.showerror("System Error", f"The authentication system encountered a critical error: {e}")
            break
